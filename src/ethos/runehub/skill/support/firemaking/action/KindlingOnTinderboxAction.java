...
public class KindlingOnTinderboxAction extends SkillAction {

    @Override
    protected void onActionStart() {
        this.getActor().getItems().deleteItem(kindling.getItemId(), 1);
        Server.itemHandler.createGroundItem(this.getActor(), kindling.getItemId(), context.getX(), context.getY(), context.getZ(), 1);
    ...
    protected void onTick() {
        try {
            ...
            if (this.isSuccessful(reaction.getLow(), reaction.getHigh())) {
                ...
                new FireBurningAction(context, ticks).execute();
                ...
                this.stop();
        		Achievements.increase(this.getActor(), AchievementType.FIRE, 1);
            }
        } catch (Exception e) {
...
//        Preconditions.checkArgument(this.getActor().inBank(),"You can not light a fire here.");
    ...
    public KindlingOnTinderboxAction(Player actor, ItemInteractionContext context, ArtisanSkillItemReaction reaction) {
        super(actor, SkillDictionary.Skill.FIREMAKING.getId(), 3);
        this.context = context;
        this.kindling = KindlingCache.getInstance().read(context.getUsedId());
