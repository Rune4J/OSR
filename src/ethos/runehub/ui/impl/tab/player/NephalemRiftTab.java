...
public class NephalemRiftTab extends TabUI {
    ...
    private void sendProgress() {
        final long instanceEndTimestamp = (RiftInstanceController.getInstance().getInstance(getPlayer().getAttributes().getInstanceId()).getInstanceStartTimestamp()
                + RiftInstanceController.getInstance().getInstance(getPlayer().getAttributes().getInstanceId()).getDurationMS());
        Server.getEventHandler().submit(new Event<>("progress-update",getPlayer(),1) {
            ...
            public void execute() {
                if (System.currentTimeMillis() >= instanceEndTimestamp) {
                    ...
                    final RiftInstance instance = RiftInstanceController.getInstance().getInstance(getPlayer().getAttributes().getInstanceId());
                    if (instance != null) {
...
//                    System.out.println("Time Remaining: " + timeRemaining);
    ...
    public NephalemRiftTab(Player player, TimedInstance instance) {
        super(57500, player, 13);
        this.titleLabel = new TextComponent(57507, "Instances");
        this.headerLabel = new TextComponent(57514, "Nephalem Rift");
