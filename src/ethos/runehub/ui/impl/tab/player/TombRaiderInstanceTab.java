...
public class TombRaiderInstanceTab extends TabUI {
    ...
    private void sendProgress() {
        final long instanceEndTimestamp = (TombInstanceController.getInstance().getInstance(getPlayer().getAttributes().getInstanceId()).getInstanceStartTimestamp()
                + TombInstanceController.getInstance().getInstance(getPlayer().getAttributes().getInstanceId()).getDurationMS());
        Server.getEventHandler().submit(new Event<>("progress-update",getPlayer(),1) {
            ...
            public void execute() {
                if (System.currentTimeMillis() >= instanceEndTimestamp) {
                    ...
                    final TombInstance instance = TombInstanceController.getInstance().getInstance(getPlayer().getAttributes().getInstanceId());
                    if (instance != null) {
    ...
    public TombRaiderInstanceTab(Player player, TimedInstance instance) {
        super(57500, player, 13);
        this.titleLabel = new TextComponent(57507, "Instances");
        this.headerLabel = new TextComponent(57514, "Tomb Raider Instance");
