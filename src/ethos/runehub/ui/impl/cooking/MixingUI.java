...
public class MixingUI extends SelectionParentUI {

    @Override
    protected void onOpen() {
        super.onOpen();
        titleLabel.setText("Item Interactions");
    ...
    private void drawListItems() {
        final List<ItemInteraction> items = ItemInteractionDAO.getInstance().getAllEntries()
                .stream()
                .filter(interaction -> interaction.getInteractionTypeId() == 2)
                .filter(interaction -> interaction.getUsedId() == context.getUsedId())
...
//                : MarkupUtils.highlightText(MarkupUtils.RED, RunehubUtils.getBooleanAsYesOrNo(item.isMembers()))));
    ...
    private void drawItems(ItemInteraction interaction) {
        ...
        items[1] = new GameItem(interaction.getUsedOnId(),1);
    }
    ...
    private void drawOption(ArtisanSkillItemReaction jewellery, ItemInteraction interaction) {
        ...
        if (this.hasRequirements(jewellery,interaction)) {
            optionOneLabel.setText(MarkupUtils.highlightText(MarkupUtils.GREEN, "Mix 1"));
            optionTwoLabel.setText(MarkupUtils.highlightText(MarkupUtils.GREEN, "Mix 5"));
            optionThreeLabel.setText(MarkupUtils.highlightText(MarkupUtils.GREEN, "Mix 10"));
            optionFourLabel.setText(MarkupUtils.highlightText(MarkupUtils.GREEN, "Mix All"));
...
//        return (jewellery.isMembers() == this.getPlayer().getAttributes().isMember()) || this.getPlayer().getAttributes().isMember();
    ...
    private void create(ArtisanSkillItemReaction jewellery, int actions, ItemInteraction interaction) {
       this.getPlayer().getSkillController().getCooking().train(new SkillItemInteractionAction(
               this.getPlayer(),
               7,
               1,
               context,
    ...
    public MixingUI(Player player, ItemInteractionContext context) {
        super(player);
