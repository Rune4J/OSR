...
//                break;
            ...
            if (c.objectX == 3079 && c.objectY == 3240) {
            		return;
            }
            c.getPA().movePlayer(3140, 3269, 1);
            break;
            
            
            // MICHAEL - PLAYERS DOWN
            case 11793:
                c.startAnimation(827);
                if (c.objectX == 3139 && c.objectY == 3267) {
                    return;
                }
                c.getPA().movePlayer(3140, 3268, 0);
                break;

       
            
            // MICHAEL - WISE OLD MANS HOUSE
            
            // END
            	
            case 2596:
            	if (obX == 3140 && obY == 3272) {
            		if (c.playerLevel[Skill.THIEVING.getId()] < 99) {
//                        c.sendMessage("You need an agility level of 99 to unlock this door.");
                        return;
                    }
                    c.getPA().movePlayer(3140, 3272, 0);
                    c.getSkillController().addXP(Skill.THIEVING.getId(), 99); // Add 15 XP to Agility skill
                }
            	break;
