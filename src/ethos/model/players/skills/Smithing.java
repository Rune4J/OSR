	...
	private static void doAction(final Player c, final int toadd, final int toremove, final int toremove2, final int timestomake, final int NOTUSED, final int NOTUSED2,
		...
		Server.getEventHandler().submit(new Event<Player>("skilling", c, 3) {
			...
			public void execute() {
				...
				c.getPA().addSkillXP((int) (xp * (c.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.SMITHING_EXPERIENCE * c.prestige())), 13, true);
	...
	private void smelt(final int barType) {
		...
		if (c.smeltAmount > 0) {
			...
			if (hasOres(barType)) {
				...
				c.getPA().addSkillXP((int) (exp * (c.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.SMITHING_EXPERIENCE * c.prestige())), Player.playerSmithing, true);
