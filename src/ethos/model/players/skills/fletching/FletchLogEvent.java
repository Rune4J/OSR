...
public class FletchLogEvent extends Event<Player> {
	...
	public void execute() {
		...
		attachment.getPA().addSkillXP((int) (fletch.getExperience() * (attachment.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.FLETCHING_EXPERIENCE * attachment.prestige())), Skill.FLETCHING.getId(), true);
		if (amount <= 0) {
			stop();
			return;
		}
