...
public class Player extends Entity implements PlayerCharacterEntity {
    ...
    public void setCurrentBookPage(int page) {
...
//    }
    ...
    public void initialize() {
        try {
            ...
            // Initialize Doors and open door 11470
            doors = new Doors();
            doors.openDefaultDoors(); // Automatically opens door 11470 if required
...
//            Membership.checkDate(this);
            ...
            if (getRightGroup().getPrimary().equals(Right.PLAYER)) {  // PLAYER RIGHTS - MICHAEL
                modHeadIcon = -1;
...
//            setSidebarInterface(2, new PlayerTabUI(this).getId()); // 638
    ...
    public void startHealthManager() {
        System.out.println("Starting health manager..."); // Debugging output
        // Initialize the HealthManager if it doesn't already exist
        if (healthManager == null) {
            healthManager = new HealthManager(this);
        }
        // Start the initial health timer
        healthManager.startLoginHealthTimer();
    }

    public void stopHealthManager() {
        if (healthManager != null) {
            healthManager.stopLoginHealthTimer();
            healthManager.stopReductionTimer();
            healthManager = null; // Clean up
        }
    }
    ...
    public int getHealthAmount() {
        return this.playerLevel[3]; // Assuming playerLevel[3] holds the health value
    }

    public void setHealthAmount(int health) {
        this.playerLevel[3] = health;
        getPA().refreshSkill(3); // Refreshes the health skill
    }
    // Method to handle item consumption, which should interact with the HealthManager
    public void handleItemConsumption(int itemId) {
        if (itemId == 2309 && healthManager != null) {
            healthManager.handleItemConsumed(itemId);
        }
    }
