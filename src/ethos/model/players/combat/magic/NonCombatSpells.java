		...
		if (System.currentTimeMillis() - c.alchDelay > 700) {
			...
			for (int i = 0; i < data.length; i++) {
				if (itemID == data[i][0]) {
					...
					c.getPA().addSkillXP((int) (data[i][5] * (c.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.SMITHING_EXPERIENCE * c.prestige())), Player.playerSmithing, true);
					c.getPA().addSkillXP((int) (53 * (c.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.MAGIC_EXP_RATE * c.prestige())), 6, true);
			...
			if (System.currentTimeMillis() - c.alchDelay > 1000) {
				if (c.getItems().playerHasItem(itemId, slot, 1)) {
					...
					if (c.getItems().playerHasItem(itemId, slot, 1)) {
						...
						c.alchDelay = System.currentTimeMillis();
						c.getPA().sendFrame106(6);
						c.getPA().addSkillXP((int) (MagicData.MAGIC_SPELLS[49][7] * (c.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.MAGIC_EXP_RATE * c.prestige())), 6, true);
			...
			if (System.currentTimeMillis() - c.alchDelay > 2000) {
				if (c.getItems().playerHasItem(itemId, slot, 1)) {
					...
					if (c.getItems().playerHasItem(itemId, slot, 1)) {
						...
						c.alchDelay = System.currentTimeMillis();
						c.getPA().sendFrame106(6);
						c.getPA().addSkillXP((int) (MagicData.MAGIC_SPELLS[50][7] * (c.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.MAGIC_EXP_RATE * c.prestige())), 6, true);
						c.getPA().refreshSkill(6);
					}
				}
			}
