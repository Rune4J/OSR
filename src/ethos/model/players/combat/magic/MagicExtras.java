		...
		if (PlayerHandler.players[playerId] != null) {
			...
			if (c.getCombat().checkMultiBarrageReqs(playerId)) {
				...
				if (Misc.random(c.getCombat().mageAtk()) > Misc.random(c2.getCombat().mageDef()) && !c.magicFailed) {
					...
					c.getPA().addSkillXP((int) (MagicData.MAGIC_SPELLS[c.oldSpellId][7] + damage * (c.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.MAGIC_EXP_RATE * c.prestige())), 6, true);
					c.getPA().addSkillXP((int) (MagicData.MAGIC_SPELLS[c.oldSpellId][7] + damage * (c.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.MAGIC_EXP_RATE * c.prestige()) / 3), 3, true);
