...
//		}
        ...
        if (NPCHandler.npcs[i] != null) {
            ...
            if (c.attackTimer <= 0) {
                ...
                c.attackTimer = c.getEquipmentInSlot(EquipmentSlot.MAIN_HAND).isPresent() ?
                        c.getEquipmentInSlot(EquipmentSlot.MAIN_HAND).get().getSpeed() :
...
//                final Optional<Equipment> equippedMainHand = c.getEquipmentInSlot(EquipmentSlot.MAIN_HAND);
//                if (equippedMainHand.isPresent()) {
//                    final Equipment equipment = equippedMainHand.get();
//                    if (RangedWeaponCache.getInstance().read(equipment.getItemId()) != null) {
//                        final RangedWeapon rangedWeapon = RangedWeaponCache.getInstance().read(equipment.getItemId());
//                        final int equippedAmmo = c.getEquipmentIdInSlot(EquipmentSlot.AMMO);
//
//                        combatType = CombatType.RANGE;
//                        if (rangedWeapon.getAcceptableProjectiles().length > 0) {
//                            if (equippedAmmo > -1) {
//                                if (Arrays.stream(rangedWeapon.getAcceptableProjectiles()).anyMatch(value -> value == equippedAmmo)) {
//
//                                } else {
//                                    c.stopMovement();
//                                    c.getCombat().resetPlayerAttack();
//                                    c.sendMessage("You do not have the right type of ammo equipped.");
//                                    return;
//                                }
//                            } else {
//                                c.stopMovement();
//                                c.getCombat().resetPlayerAttack();
//                                c.sendMessage("You must have ammo equipped to use this.");
//                                return;
//                            }
//                        }
//                    }
//                }
                //TODO Old ranged ammo check code
                ...
                if (Arrays.stream(RunehubConstants.CBOW).anyMatch(value -> c.playerEquipment[c.playerWeapon] == value) || c.usingBow || c.usingOtherRangeWeapons || c.usingCross || c.usingBallista) {
                    combatType = CombatType.RANGE;
                ...
                if (combatType == CombatType.RANGE) {//if (c.usingBow && !c.usingOtherRangeWeapons && !c.usingMagic || c.usingCross || c.usingBallista) { // range
                    if (c.usingCross)
                        c.usingBow = true;
                    if (c.fightMode == 2)
                        c.attackTimer--;
                    ...
                    c.lastArrowUsed = c.playerEquipment[c.playerArrows];
                    c.lastWeaponUsed = c.playerEquipment[c.playerWeapon];
                    int startGFX = c.getCombat().getRangeStartGFX();
                    try {startGFX = ProjectileCache.getInstance().read(c.lastArrowUsed).getStartGfx();} catch (NullPointerException e) {}
//                    System.out.println("Projectile Start: " + startGFX);
                    c.gfx100(startGFX);
                    c.hitDelay = c.getCombat().getHitDelay(i, ItemAssistant.getItemName(c.playerEquipment[c.playerWeapon]).toLowerCase());
                    c.projectileStage = 1;
                    c.oldNpcIndex = i;
...
//                        System.out.println("Projectile: " + RangeData.getRangeProjectileGFX(c));
                        c.getCombat().fireProjectileNpc(0);
                    }
                }
